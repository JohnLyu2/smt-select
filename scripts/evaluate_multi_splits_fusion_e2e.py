#!/usr/bin/env python3
"""
Evaluate Fusion-E2E-PWC (fine-tuned GIN backbone + frozen text embeddings + PWC heads)
...
Expects:
  ...
  - Pre-trained GIN-PWC models: --gin-models-base/gin_pwc/{division}/seed{N}/ (config.json, model.pt, vocab.json).
  - Text embeddings: --desc-features-dir/{division}/features.csv.

For each seed: load pre-trained GIN backbone, build graphs, load text embeddings,
train Fusion-E2E-PWC, evaluate on train and test, write summary and per-seed CSVs.
Use --eval-only with --models-base to evaluate pre-trained fusion-e2e models without training.
"""
...
from src.fusion_e2e_pwc import (
    FusionE2EPwcSelector,
    train_fusion_e2e_pwc,
)
...
def _build_text_by_full_path(
    desc_csv: Path,
    benchmark_root: Path,
    paths_full: list[str],
) -> dict[str, np.ndarray]:
    """Load text embeddings CSV and map to full paths."""
...
def evaluate_multi_splits_fusion_e2e(
    ...
    *,
    ...
    """
    Run Fusion-E2E-PWC train/eval for each seed under splits_dir.
    When eval_only=True, load models from models_base/fusion_e2e_pwc/{division}/seed{N}.
    """
    ...
    if not desc_csv.is_file():
        raise FileNotFoundError(f"Description features CSV not found: {desc_csv}")
    ...
    logging.info(
        "Starting Fusion-E2E-PWC evaluation over %d splits for division %s%s",
        n_seeds, division, " (eval-only)" if eval_only else "",
    )
    ...
    for seed_val, seed_dir in seed_entries:
        ...
        # Text embeddings (full paths)
        paths_full = list(train_data.keys()) + list(test_data.keys())
        text_by_path = _build_text_by_full_path(desc_csv, root, paths_full)

        # GIN extraction overhead (from the pre-trained GIN-PWC extraction times)
        gin_extraction_csv = gin_base / "gin_pwc" / division / f"seed{seed_val}" / "extraction_times.csv"
        if gin_extraction_csv.is_file():
            gin_extraction_rel = load_extraction_times_csv(gin_extraction_csv)
            gin_overhead_by_full = {str(root / p): t for p, t in gin_extraction_rel.items()}
        else:
            gin_overhead_by_full = {}
        ...
        logging.info(
            "Instances: %d train, %d test; %d with text embeddings",
            n_train, n_test, len(text_by_path),
        )

        if eval_only:
            model_save_dir = models_base / "fusion_e2e_pwc" / division / f"seed{seed_val}"
            ...
            # Pre-trained GIN-PWC model dir for this seed
            pretrained_gin_dir = gin_base / "gin_pwc" / division / f"seed{seed_val}"
            if not pretrained_gin_dir.is_dir() or not (pretrained_gin_dir / "config.json").exists():
                raise FileNotFoundError(
                    f"Pre-trained GIN-PWC model not found: {pretrained_gin_dir}. "
                    "Train GIN-PWC first with evaluate_multi_splits_gin.py."
                )

            if save_models and models_base is not None:
                model_save_dir = models_base / "fusion_e2e_pwc" / division / f"seed{seed_val}"
            elif save_models and output_dir:
                model_save_dir = output_dir / "models" / "fusion_e2e_pwc" / division / f"seed{seed_val}"
            else:
            ...
            try:
                train_fusion_e2e_pwc(
                    pretrained_gin_dir=str(pretrained_gin_dir),
                    text_by_path=text_by_path,
                    multi_perf_data=train_data_for_training,
        ...
        selector = FusionE2EPwcSelector.load(
            str(model_save_dir),
            text_by_path=text_by_path,
            device="cpu",
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Evaluate Fusion-E2E-PWC (fine-tuned GIN + frozen text + PWC) over multiple splits"
    )
    ...
    parser.add_argument(
        "--gin-models-base", type=str, default=None,
        help=f"Base for pre-trained GIN-PWC models (default: {DEFAULT_GIN_MODELS_BASE})",
    )
    parser.add_argument(
        "--desc-features-dir", type=str, default=None,
    ...
    parser.add_argument("--graph-timeout", type=int, default=5, help="Graph build timeout in seconds (default: 5)")
    parser.add_argument("--jobs", type=int, default=8, help="Parallel workers for graph building (default: 8)")
    ...
    parser.add_argument(
        "--skip-easy-unsolvable", action="store_true",
        help="Exclude train instances where no solver solves or all solve in <= N seconds",
    )
    parser.add_argument(
        "--skip-trivial-under", type=float, default=24.0,
        help="When --skip-easy-unsolvable: exclude trivial instances (default 24)",
    )
    ...
    evaluate_multi_splits_fusion_e2e(
